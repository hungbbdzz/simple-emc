...
import com.velorise.simpleemc.network.ClientPayloadHandler;
import com.velorise.simpleemc.network.RequestWithdrawPayload;
import com.velorise.simpleemc.network.ServerPayloadHandler;
import com.velorise.simpleemc.network.SyncPlayerEMCPayload;
...
public class SimpleEMC {
    ...
    private void registerPayloads(final RegisterPayloadHandlersEvent event) {
        ...
        registrar.playToClient(
            com.velorise.simpleemc.network.SyncCustomEMCPayload.TYPE,
            com.velorise.simpleemc.network.SyncCustomEMCPayload.STREAM_CODEC,
            ClientPayloadHandler::handleCustomEMCSync
        );

        // Client-bound open config UI packet (Server -> Client)
        registrar.playToClient(
            com.velorise.simpleemc.network.OpenConfigScreenPayload.TYPE,
            com.velorise.simpleemc.network.OpenConfigScreenPayload.STREAM_CODEC,
            ClientPayloadHandler::handleOpenConfigScreen
        );

        // Server-bound custom EMC edit packet (Client -> Server)
        registrar.playToServer(
            com.velorise.simpleemc.network.RequestUpdateEMCPayload.TYPE,
            com.velorise.simpleemc.network.RequestUpdateEMCPayload.STREAM_CODEC,
        ...
        registrar.playToServer(
            com.velorise.simpleemc.network.FillCraftingFromEMCPayload.TYPE,
            com.velorise.simpleemc.network.FillCraftingFromEMCPayload.STREAM_CODEC,
    ...
    public static void syncCustomEMCOverrides(ServerPlayer player) {
        ...
        PacketDistributor.sendToPlayer(player, new com.velorise.simpleemc.network.SyncCustomEMCPayload(fullOverrides));
    }
            ...
            dispatcher.register(net.minecraft.commands.Commands.literal("simpleemc")
                ...
                .then(net.minecraft.commands.Commands.literal("config")
                    ...
                    .executes(context -> {
                        ...
                        PacketDistributor.sendToPlayer(player, new com.velorise.simpleemc.network.OpenConfigScreenPayload());
                        return 1;
                    })
                )
            );
